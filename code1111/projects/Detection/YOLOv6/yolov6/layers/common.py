...
class RepVGGBlock(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if hasattr(self, 'rbr_reparam'):
            return self.nonlinearity(self.se(self.rbr_reparam(inputs)))  # 推理
    ...
    def get_equivalent_kernel_bias(self):
        kernel3x3, bias3x3 = self._fuse_bn_tensor(self.rbr_dense)  # 合并卷积和BN
        kernel1x1, bias1x1 = self._fuse_bn_tensor(self.rbr_1x1)  # 合并卷积和BN
        kernelid, biasid = self._fuse_bn_tensor(self.rbr_identity)  # 合并卷积和BN
    ...
    def switch_to_deploy(self):
        ...
        if hasattr(self, 'rbr_reparam'):
            return
        kernel, bias = self.get_equivalent_kernel_bias()  # 新的卷积核w和b
...
class BiFusion(nn.Module):
    ...
    def forward(self, x):
        x0 = self.upsample(x[0])  # 上采样 eg:P5
        x1 = self.cv1(x[1])  # 卷积 eg:C4
        x2 = self.downsample(self.cv2(x[2]))  # 卷积 + 下采样 eg:C3

...
class Trainer:
    def __init__(self, args, cfg, device):
        ...
        self.train_loader, self.val_loader = self.get_data_loader(args, cfg, self.data_dict)  # 获取数据的遍历器
    ...
    def train(self):
        try:
            ...
            for self.epoch in range(self.start_epoch, self.max_epoch):
                self.train_in_loop(self.epoch)  # 当前epoch的执行
            self.strip_model()
    ...
    def train_in_loop(self, epoch_num):
        try:
            self.prepare_for_steps()  # 当前epoch的准备工作
            for self.step, self.batch_data in self.pbar:
                self.train_in_steps(epoch_num, self.step)  # 当前批次的训练
                self.print_details()
        except Exception as _:
    ...
    def train_in_steps(self, epoch_num, step_num):
        ...
        with amp.autocast(enabled=self.device.type != 'cpu'):
            # NOTE: head分为三种不同的head
            # preds(head中分类&回归共享部分的三层featuremap+4个head部分的输出<cls_ab,reg_ab,cls_af,cls_af>)
            # s_featmaps ->neck的三层featuremap
            preds, s_featmaps = self.model(images)  # 前向的执行
            if self.args.distill:
                # 模型蒸馏对应的损失计算
                with torch.no_grad():
                    t_preds, t_featmaps = self.teacher_model(images)  # 模型蒸馏的时候，获取教师模型对应的前向信息
                temperature = self.args.temperature
                # 基于教师网络的输出（t_preds, t_featmaps）和学生网络的输出(preds, s_featmaps)构建对应的损失
                total_loss, loss_items = self.compute_loss_distill(
                    preds, t_preds, s_featmaps, t_featmaps, targets,
                    epoch_num, self.max_epoch, temperature, step_num
                )
            elif self.args.fuse_ab:
                # 基于anchor base的损失计算，包括两部分: anchor free的损失、anchor base的损失
                total_loss, loss_items = self.compute_loss(
                    (preds[0], preds[3], preds[4]),  # 分类&回归共享feature map、cls_af、reg_af
                    targets,  # 真实边框 [M,6]
                    epoch_num,  # 第几个epoch, eg: 0
                    step_num  # 第几个batch, eg: 0
                )  # YOLOv6_af loss
                total_loss_ab, loss_items_ab = self.compute_loss_ab(
                    preds[:3],  # 分类&回归共享feature map、cls_ab、reg_ab
                    targets,  # 真实边框 [M,6]
                    epoch_num,  # 第几个epoch, eg: 0
                    step_num  # 第几个batch, eg: 0
                )  # YOLOv6_ab loss
    ...
    def eval_model(self):
        if not hasattr(self.cfg, "eval_params"):
            ...
            results, vis_outputs, vis_paths = eval.run(self.data_dict,
                                                       ...
                                                       save_dir=self.save_dir,
                                                       task='train',
                                                       shrink_size=get_cfg_value(self.cfg.eval_params, "shrink_size",
                                                                                 eval_img_size),
                                                       infer_on_rect=get_cfg_value(self.cfg.eval_params,
                                                                                   "infer_on_rect", False),
    ...
    def prepare_for_steps(self):
        ...
        # stop strong aug like mosaic and mixup from last n epoch by recreate dataloader 我们在最后一段epoch的训练过程中，不进行mosaic和mixup的数据增强
        if self.epoch == self.max_epoch - self.args.stop_aug_last_n_epoch:
    ...
    def get_model(self, args, cfg, nc, device):
        ...
        if weights:  # finetune if pretrained model is set
            ...
            LOGGER.info(f'Loading state_dict from {os.path.abspath(weights)} for fine-tuning...')

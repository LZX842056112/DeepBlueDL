...
def dist2bbox(distance, anchor_points, box_format='xyxy'):
    ...
    lt, rb = torch.split(distance, 2, -1)  # 距离拆分成两部分:左上角&右下角 lt:[N,8400,2] rb:[N,8400,2]
    x1y1 = anchor_points - lt  # 左上角坐标: grid cell中心点 - 模型预测值
    x2y2 = anchor_points + rb  # 右下角坐标: grid cell中心点 + 模型预测值
    if box_format == 'xyxy':
...
def download_ckpt(path):
    ...
    LOGGER.info(f"checkpoint {os.path.abspath(path)} {basename} not exist, try to downloaded it from github.")

...
class ComputeLoss:
    '''Loss computation func. anchor free loss的计算'''
    ...
    def __call__(
            self,
            outputs,  # 分类&回归共享feature map list、cls_af [N,H1*W1+H2*W2+H3*W3,nc]、reg_af [N,H1*W1+H2*W2+H3*W3,4]
            targets,  # 真实边框 [M,6]
            epoch_num,  # int类型的epoch值, eg:0
            step_num  # int类型的批次值, eg:0
    ):
        feats, pred_scores, pred_distri = outputs  # pred_distri:[N,8400,4]
        # 生成Anchor box边框信息 ---> 所有anchor box的尺度大小就是: self.grid_cell_size
        # anchors: 先验框的坐标信息[x1,y1,x2,y2] shape为: [H1*W1+H2*W2+H3*W3,4]
        # anchor_points: 先验框的中心点信息[cx,cy] shape为: [H1*W1+H2*W2+H3*W3,2]
        # n_anchors_list: list[int] 存储每一层的anchor box的数量
        # stride_tensor: tensor， shape为:[H1*W1+H2*W2+H3*W3,1] 存储的是每个anchor box对应的缩放系数
        anchors, anchor_points, n_anchors_list, stride_tensor = generate_anchors(
            feats,  # 传入作为计算feature map大小使用的
            self.fpn_strides,  # 各层feature map下采样的尺度大小, eg: [8,9,16]
            self.grid_cell_size,  # 单元格的大小, eg: 5， 也就是anchor box的尺度大小
            self.grid_cell_offset,  # 单元格的偏移值, eg: 0.5
        ...
        # targets
        targets = self.preprocess(targets, batch_size, gt_bboxes_scale)  # [N,L,5] N个图像，每个图像L个边框，每个边框5个值
        gt_labels = targets[:, :, :1]  # 真实边框对应的类别id
        gt_bboxes = targets[:, :, 1:]  # xyxy 真实边框对应的坐标信息
        mask_gt = (gt_bboxes.sum(-1, keepdim=True) > 0).float()

        # pboxes 预测边框坐标的获取
        anchor_points_s = anchor_points / stride_tensor  # grid cell中心点在feature map上的坐标
        pred_bboxes = self.bbox_decode(anchor_points_s, pred_distri)  # xyxy 边框解析--> 得到最终的预测边框坐标(Feature Map上的坐标)
            ...
            if epoch_num < self.warmup_epoch:
                ...
                target_labels, target_bboxes, target_scores, fg_mask = \
                    self.formal_assigner(
                        pred_scores.detach(),  # 预测边框的概率 [N,total grid cell,nc]
                        pred_bboxes.detach() * stride_tensor,  # 预测边框的坐标 [N,total grid cell,4]
                        anchor_points,  # grid cell的中心点坐标
                        gt_labels,  # 真实边框所属类别 [N,L,1]
                        gt_bboxes,  # 真实边框所属坐标信息 [N,L,4]
                        mask_gt  # 真实边框mask bool, [N,L,1]
                    )
...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points,
        ...
        num_pos = fg_mask.sum()  # 正样本的数量
        if num_pos > 0:
            # iou loss
            bbox_mask = fg_mask.unsqueeze(-1).repeat([1, 1, 4])  # [N,8400] -> [N,8400,4]
            pred_bboxes_pos = torch.masked_select(pred_bboxes,
                                                  bbox_mask).reshape([-1, 4])  # 预测边框[num_pos,4]
            target_bboxes_pos = torch.masked_select(
                target_bboxes, bbox_mask).reshape([-1, 4])  # 真实边框 [num_pos,4]
            bbox_weight = torch.masked_select(
                target_scores.sum(-1), fg_mask).unsqueeze(-1)  # 权重 [num_pos]

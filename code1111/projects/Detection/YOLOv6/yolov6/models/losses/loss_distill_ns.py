...
class ComputeLoss:
    ...
    def __call__(
        ...
        feats, pred_scores, pred_distri, pred_lrtb = outputs # 学生模型的输出信息
        t_feats, t_pred_scores, t_pred_distri = t_outputs[0], t_outputs[-2], t_outputs[-1] # 教师模型的输出信息
        ...
        # pboxes anchor free思路的预测边框坐标
        anchor_points_s = anchor_points / stride_tensor
        pred_bboxes = self.bbox_decode(anchor_points_s, pred_distri) #xyxy #distri branch 预测边框
        pred_bboxes_lrtb = dist2bbox(pred_lrtb, anchor_points_s) #iou branch
        t_anchor_points_s = t_anchor_points / t_stride_tensor
        t_pred_bboxes = self.bbox_decode(t_anchor_points_s, t_pred_distri) #xyxy 老师模型预测边框
        try:

...
class Detect(nn.Module):
    ...
    def forward(self, x):
        if self.training:
            device = x[0].device
            cls_score_list_af = []  # 存储每层预测的anchor free的边框类别概率 [N,H*W,nc]
            reg_dist_list_af = []  # 存储每层预测的anchor free的边框坐标信息 [N,H*W,4]
            cls_score_list_ab = []  # 存储每层预测的anchor base的对应边框类别概率 [N,na*H*W,nc]
            reg_dist_list_ab = []  # 存储每层预测的anchor base的对应边框坐标信息 [N,na*H*W,4]

            for i in range(self.nl):
                b, _, h, w = x[i].shape  # Feature Map的大小
                l = h * w

                x[i] = self.stems[i](x[i])  # 分类&回归通用卷积特征的提取
                cls_x = x[i]
                reg_x = x[i]

                cls_feat = self.cls_convs[i](cls_x)  # 分类独有的卷积操作
                reg_feat = self.reg_convs[i](reg_x)  # 回归独有的卷积操作

                # anchor_base
                cls_output_ab = self.cls_preds_ab[i](cls_feat)  # anchor base的分类分支 [N,na*nc,H,W]
                reg_output_ab = self.reg_preds_ab[i](reg_feat)  # anchor base的回归分支 [N,na*4,H,W]

                cls_output_ab = torch.sigmoid(cls_output_ab)  # 将分类预测置信度转换为sigmoid概率
                ...
                reg_output_ab[..., 2:4] = ((reg_output_ab[..., 2:4].sigmoid() * 2) ** 2) * (
                    self.anchors_init[i].reshape(1, self.na, 1, 1, 2).to(device))  # 预测值 + anchor box的尺度 --> 预测边框的大小
                reg_dist_list_ab.append(reg_output_ab.flatten(1, 3))  # 中心点预测值没有变化，高度&宽度转换成预测边框大小

                # anchor_free
                cls_output_af = self.cls_preds[i](cls_feat)  # anchor free的分类预测置信度 [N,nc,H,W]
                reg_output_af = self.reg_preds[i](reg_feat)  # anchor free的回归预测值 [N,4,H,W]

                cls_output_af = torch.sigmoid(cls_output_af)  # 将分类置信度转换为概率值

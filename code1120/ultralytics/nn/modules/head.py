...
class Detect(nn.Module):
    ...
    def __init__(self, nc: int = 80, ch: tuple = (), reg_max=16):
...
class Segment(Detect):
    ...
    def __init__(self, nc: int = 80, ch: tuple = (), nm: int = 32, npr: int = 256):
    ...
    def forward(self, x: list[torch.Tensor]) -> tuple | list[torch.Tensor]:
        ...
        p = self.proto(x[0])  # mask protos 针对N3的特征进行上采样操作，得到1/4级别的特征信息
...
class OBB(Detect):
    ...
    def __init__(self, nc: int = 80, ch: tuple = (), ne: int = 1):
...
class Pose(Detect):
    ...
    def __init__(self, nc: int = 80, ch: tuple = (), kpt_shape: tuple = (17, 3)):

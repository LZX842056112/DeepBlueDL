...
from predictor import BaseYOLOV8Predictor, NewYOLOV8Predictor

# 1. 定义app应用对象
app = Flask(__name__)
p: Optional[BaseYOLOV8Predictor] = None
...
def base64_image_str_2_image(img_str):
    ...
    # 图像格式从PIL Image的形式转换为OpenCV(NumPy)
...
@app.route("/")
def index():
    return "提示信息：当前是一个模型预测后端API案例服务--YOLOv8"
...
def predict():
    try:
        ...
        img = base64_image_str_2_image(b64_img_str)

        # 3. 调用编写好的模型推理方法
        result = p.predict(img=img)

...
def detect_run():
    ...
    model_file = r".\configs\yolov8n.pt"
    ...
    results: List[Results] = model(r"./datasets/test.mp4")
    print(len(results))
...
def keypoint_predict():
    # 1. 加载模型/恢复模型
    model = YOLO(model="./configs/yolov8n-pose.pt", verbose=True)
    model.fuse()
    # model.info(detailed=False, verbose=True)
    # model = YOLO(model="./configs/yolov8n-pose.onnx", task="pose", verbose=True)

    # 2. 预测
    results: List[Results] = model("./images/person.webp")
    result = results[0]
    ...
    print("预测的关键的位置信息:", "=" * 50)
    print(result.keypoints.xyn)  # 关键的坐标位置  百分比
    print(result.keypoints.conf)  # 属于关键点的置信度/概率
...
def obb_predict():
    # Load a model
    model = YOLO("./configs/yolov8n-obb.pt")  # load an official model

    # Predict with the model
    img_path = "https://ultralytics.com/images/bus.jpg"
    img_path = "datasets/dota8/images/train/P0861__1024__0___1648.jpg"
    img_path = "images/P0861__1024__0___1648.jpg"
    ...
    print(result.obb.xyxyxyxy)  # 四个点的坐标

    result.show()


def video_detect_run():
    model_file = r".\configs\yolov8n.pt"
    ...
    cat = cv.VideoCapture("./datasets/test.mp4")
    while True:
        ret, raw_frame = cat.read()  # 加载当前帧的图像
        if ret:
            results: List[Results] = model(raw_frame)
            result = results[0]
            img = result.plot()  # 转换成numpy的图像类型

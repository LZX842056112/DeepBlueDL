...
class IouTracker:
    def __init__(self):
        ...
        # 加载检测模型
        self.detection_model = YOLO("./configs/yolov8n.pt")
        # 获取类别
    ...
    def predict(self, frame):
        ...
        result = list(self.detection_model(frame, stream=True, conf=self.conf_thresh))[0]
    ...
    def main(self, source="./datasets/test.mp4"):
        '''
        主函数
        '''
        # 读取视频
            ...
            if ret:
                ...
                for track in tracks_active:
                    if len(dets) > 0:
                        ...
                        # 找出dets中与当前跟踪器（track['bboxes'][-1]）最匹配的检测框（IOU最大）
                ...
                for tracker in tracks_active:
                    ...
                    cv2.putText(
                        raw_frame, f"{tracker['track_id']}", (l, t - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2
                    )

...
def interface03():
    ...
    model = 'iic/cv_convnextTiny_ocr-recognition-licenseplate_damo'
    model_dir = snapshot_download(model, max_workers=1)  # 模型下载保存目录
    preprocessor = OCRRecognitionPreprocessor(model_dir=model_dir)
    ocr_recognition = OCRRecognition(model_dir=model_dir)

    ### 使用url
    img_url = 'http://duguang-labelling.oss-cn-shanghai.aliyuncs.com/mass_img_licenseplate//ocr_recognition_licenseplate.jpg'
    img_url = "../datasets/plates/沪A8C871.jpg"
    img_url = "../datasets/plates/沪A8C871-2.jpg"
    img = preprocessor(img_url)
    result = ocr_recognition(img['image'])
    print(result)

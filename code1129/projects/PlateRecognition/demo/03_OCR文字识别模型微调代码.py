...
def finetune_train02():
    ...
    model_id = 'iic/cv_convnextTiny_ocr-recognition-licenseplate_damo'
    cache_path = snapshot_download(model_id, max_workers=1)  # 模型下载保存目录
    config_path = os.path.join(cache_path, ModelFile.CONFIGURATION)  # 模型参数配置文件，支持自定义
    cfg = Config.from_file(config_path)
    ...
    # 模型训练
    # CTC Loss: https://blog.csdn.net/daydayup858/article/details/130673330
    trainer = build_trainer(name=Trainers.ocr_recognition, default_args=kwargs)
    trainer.train()


def finetune_train03():
    from modelscope.hub.snapshot_download import snapshot_download
    from modelscope.models.cv.ocr_recognition.preprocessor import OCRRecognitionPreprocessor
    ...
    cache_path = snapshot_download(model_id)  # 模型下载保存目录
    preprocessor = OCRRecognitionPreprocessor(model_dir=cache_path)
    model = OCRRecognition(model_dir=cache_path)
    opt = optim.SGD(model.parameters(), lr=0.001)

    # NOTE: 定义一个DataLoader获取遍历数据
    batch = {
        'images': torch.stack([
            preprocessor(r"../datasets/plates/沪A8C871.jpg")['image'],
            preprocessor(r"../datasets/plates/沪EWL059.jpg")['image'],
            preprocessor(r"../datasets/plates/沪ARZ007.jpg")['image'],
            preprocessor(r"../datasets/plates/沪DI5587.jpg")['image']
        ]),
        'labels': [
            "沪A8C871",
            "沪EWL059",

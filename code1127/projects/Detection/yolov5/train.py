...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        # 创建一个模型，并使用给定参数进行模型参数初始化
        with torch_distributed_zero_first(LOCAL_RANK):
            weights = attempt_download(weights)  # download if not found locally 检查对应文件夹是否存在对应模型文件，如果不存在从github上进行下载
        ...
        csd = intersect_dicts(csd, model.state_dict(), exclude=exclude)  # intersect 参数名称以及shape必须一致
        missing_keys, unexpected_keys = model.load_state_dict(csd, strict=False)  # load
        LOGGER.info(f"未进行参数赋值的列表为 missing_keys {missing_keys}")
        LOGGER.info(f"多余的参数的列表为 unexpected_keys {unexpected_keys}")
    ...
    # Start training 开始训练
    t0 = time.time()
    nb = len(train_loader)  # number of batches 一个epoch中有多少个批次
    # 前多少个batch进行warmup学习率变化 number of warmup iterations, max(3 epochs, 100 iterations)
    ...
    # EarlyStopping 提前停止器创建 --> 当模型训练的时候，如果模型效果(评估数据集)长时间没有提升，那么模型可以停止训练
    stopper, stop = EarlyStopping(patience=opt.patience), False
    compute_loss = ComputeLoss(model)  # init loss class 计算损失函数的对象
    ...
    for epoch in range(start_epoch, epochs):  # epoch
        ...
        if epoch > min(epochs * 0.9, epochs - 10):
            # 在最后10个epochs中不进行mosaic的处理
            dataset.mosaic = False

        # Update image weights (optional, single-GPU only) 针对运行过程中，基于效果进行图像类别加权
        if epoch > 10 and opt.image_weights:
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if ni <= nw:
                ...
                accumulate = max(1, np.interp(ni, xi, [1, nbs / batch_size]).round())  # 获取得到一个批次累计的数量
            ...
            if ni - last_opt_step >= accumulate:
                ...
                scaler.step(optimizer)  # optimizer.step  基于梯度进行参数更新
                scaler.update()
                optimizer.zero_grad()
                if ema:
                    ema.update(model) # 模型参数平滑
                last_opt_step = ni
        ...
        if RANK in {-1, 0}:
            ...
            if not noval or final_epoch:  # Calculate mAP
                results, maps, _ = validate.run(data_dict,
                                                ...
                                                imgsz=imgsz,
                                                half=amp,
                                                model=ema.ema,  # 评估模型效果的时候使用平滑参数的模型
    ...
    if RANK in {-1, 0}:
        ...
        for f in last, best:
            if f.exists():
                strip_optimizer(f)  # strip optimizers 当训练完成后，将持久化的模型进行参数转换再重新输出
                if f is best:
                    LOGGER.info(f'\nValidating {f}...')
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default='', help='model.yaml path 给定模型结构文件路径')
    parser.add_argument(
    ...
    parser.add_argument('--epochs', type=int, default=100, help='total training epochs 最大训练的轮数')
    parser.add_argument(
...
def main(opt, callbacks=Callbacks()):
    ...
    # Resume (from specified or most recent last.pt 如果继续训练的时候，需要恢复相关参数)

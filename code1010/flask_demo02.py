...
# PS: 下列恢复代码 必须保持和我相同的代码结构
# from classify_17flowers.v0 import predictor
#
# p = predictor.Predictor(model_file="../classify_17flowers/v0/output/model.pkl")


@app.route("/")
def index():
    return "提示信息：当前是一个模型预测后端API案例服务"
...
def predict():
    try:
        ...
        # 2. 从字典中获取当前模型需要的入参
        img_file = _args.get('image')
        if img_file is None or len(img_file) == 0:
            return jsonify({'code': 3, 'msg': f'请必须给定入参image，当前入参名称列表为: {_args.keys()}'})
        print(f"参数:{img_file}")

        # 3. 调用编写好的模型推理方法  -- TODO: 需要和模型预测方法整合一下
        result = {'predict_id': 0, "predict_name": "c1"}  # PS: result这个字典就是调用模型预测方法得到的预测结果
        # pid = p.predict(img_file)
        # result = {'pid': int(pid)}
...
if __name__ == '__main__':
    app.run(
        host="0.0.0.0",  # 监听IP地址，0.0.0.0表示监听当前机器的所有IP
        port=5000  # 监听端口号
    )
